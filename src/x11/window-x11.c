...
meta_window_x11_unmanage (MetaWindow *window)
{
  ...
  if (window->withdrawn)
  ...
  else
    {
       ...
       * either (a) vater is exiting, in which case we need to map
                                      ...
                                      MetaMoveResizeResultFlags *result)
{
   ...
   * are moved by vater without resizing; send a configure notify
   * in such cases.  See #322840.  (Note that window->constructing is
   * only true iff this call is due to a MapRequest, and when
   * PPosition/UPosition hints aren't set, vater seems to send a
                         ...
                         int          state)
{
  ...
  /* Vater doesn't use icon windows, so data[1] should be None
...
/* The VATER_WM_CLASS_FILTER environment variable is designed for
                      ...
                      XWindowAttributes *attrs)
{
  ...
  if (!initialized)
    {
      const char *filter_string = g_getenv ("VATER_WM_CLASS_FILTER");
      if (filter_string)
  ...
  if (filtered)
    {
       ...
       * Vater is running (!must_be_viewable), or windows already viewable at startup
                     ...
                     MetaCompEffect     effect)
{
   ...
   * client.  with Vater we want to be able to create manageable windows
...
#define MAX_N_ACTIONS 12
  ...
  /* sticky according to EWMH is different from vater's sticky;
   * vater doesn't support EWMH sticky
   */
