...
static void
reload_vater_hints (MetaWindow    *window,
                     MetaPropValue *value,
                     gboolean       initial)
{
  if (value->type != META_PROP_VALUE_INVALID)
    {
      ...
      char     *old_hints = window->vater_hints;
      ...
      if (changed)
        {
          ...
          if (new_hints)
            window->vater_hints = g_strdup (new_hints);
          else
            window->vater_hints = NULL;

          g_object_notify (G_OBJECT (window), "vater-hints");
        }
    }
  else if (window->vater_hints)
    {
      g_free (window->vater_hints);
      window->vater_hints = NULL;
...
meta_x11_display_init_window_prop_hooks (MetaX11Display *x11_display)
{
  ...
  MetaWindowPropHooks hooks[] = {
    ...
    { x11_display->atom__VATER_HINTS,     META_PROP_VALUE_TEXT_PROPERTY, reload_vater_hints, LOAD_INIT | INCLUDE_OR },
